...
DOCKER_VOLUME="${DOCKER_VOLUME} -v $(dirname ${RUN_DIR}):/workspace/acmnet:rw"
DOCKER_VOLUME="${DOCKER_VOLUME} -v ${XSOCK}:${XSOCK}:rw"
DOCKER_VOLUME="${DOCKER_VOLUME} -v ${XAUTH}:${XAUTH}:rw"
if [[ ${DATASET_DIR} != "" ]]; then
  DOCKER_VOLUME="${DOCKER_VOLUME} -v ${DATASET_DIR}:/workspace/acmnet/datasets:rw"
fi
