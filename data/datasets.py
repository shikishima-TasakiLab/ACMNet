...
class H5_Train_Dataset(HDF5Dataset):
    def __init__(self, h5_paths: List[str], config:str, quiet:bool=True, block_size:int=0, use_mods: Tuple[int, int]=None,
        ...
        super(H5_Train_Dataset, self).__init__(h5_paths, config, quiet, block_size, use_mods, visibility_filter_radius, visibility_filter_threshold)
    ...
    def __getitem__(self, index: int) -> Dict[str, torch.Tensor]:
        ...
        if self.depth_norm_aug_rate is not None:
            self.depth_norm_c += (np.random.rand() * 2.0 - 1.0) * self.depth_norm_aug_rate
        # Random Pose Error
        ...
        if self.minibatch['sparse'][CONFIG_TAG_NORMALIZE] is True:
            tr_norm *= self.minibatch['sparse'][CONFIG_TAG_RANGE][1] * self.depth_norm_c
        return items
    ...
    def depth_common(self, src: np.ndarray, minibatch_config: Dict[str, Union[str, Dict[str, str], List[int], bool, List[float]]]) -> np.ndarray:
        ...
        if minibatch_config[CONFIG_TAG_NORMALIZE] is True:
            ...
            dst = np.where(range_max < dst, NORMALIZE_INF, (dst - range_min) / (range_max - range_min))
...
class H5_Test_Dataset(HDF5Dataset):
    def __init__(self, h5_paths:List[str], config:str, quiet:bool=True, block_size:int=0, use_mods:Tuple[int, int]=None,
        ...
        super(H5_Test_Dataset, self).__init__(h5_paths, config, quiet, block_size, use_mods, visibility_filter_radius, visibility_filter_threshold)
    ...
    def __vec2quat(self, vec: np.ndarray, abs: float) -> np.ndarray:
        # Rotation vector to Quaternion.
        xyz: np.ndarray = vec * np.sin(np.repeat(abs[:,np.newaxis], 3, axis=1) * 0.5)
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        if self.random_pose is True:
            items['pose_err'] = torch.from_numpy(CONVERT_TORCH[TYPE_POSE](DTYPE_TORCH[TYPE_POSE](np.concatenate([self.tr_err / tr_norm, self.q_err]))))
        else:
